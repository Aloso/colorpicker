...
function updateLutTexture(
  ...
  gl.texImage2D(
    gl.TEXTURE_2D,
    0, // Mipmap level
    gl.RGB8, // Internal format (GPU side)
    size, // Width
    size, // Height
    0, // Border (must be 0)
    gl.RGB, // Format of source data
    gl.UNSIGNED_BYTE, // Data type of source data
    data, // Your Uint8Array
  );
}
...
export function renderSquare(shader: Shader, getters: Getters) {
  return function renderCanvas(canvas: HTMLCanvasElement) {
    ...
    const render = () => {
      ...
      if (getters.lut && lutTexture) {
        ...
        gl.bindTexture(gl.TEXTURE_2D, lutTexture);
        updateLutTexture(gl, getters.lut(), 128);
