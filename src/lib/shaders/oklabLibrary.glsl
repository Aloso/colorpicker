...
float srgb_transfer_function(float a) {
  return 0.0031308 >= a ? 12.92 * a
                        : 1.055 * pow(a, 0.4166666666666667) - 0.055;
}

float srgb_transfer_function_inv(float a) {
  return 0.04045 < a ? pow((a + 0.055) / 1.055, 2.4) : a / 12.92;
}
  ...
  if (((L1 - L0) * cusp.y - (cusp.x - L0) * C1) <= 0.0) {
    ...
    // Then one step Halley's method
    {
      ...
      // block can be used:
      {
        ...
        float b1 =
            -0.0041960863 * ldt - 0.7034186147 * mdt + 1.7076147010 * sdt;
        float b2 =
...
vec3 gamut_clip_preserve_chroma(vec3 rgb) {
  if (rgb.r < 1.0 && rgb.g < 1.0 && rgb.b < 1.0 && rgb.r > 0.0 && rgb.g > 0.0 &&
      rgb.b > 0.0)
    return rgb;
...
vec3 gamut_clip_project_to_L_cusp(vec3 rgb) {
  ...
  // The cusp is computed here and in find_gamut_intersection, an optimized
  // solution would only compute it once.
...
vec3 gamut_clip_adaptive_L0_L_cusp(vec3 rgb, float alpha) {
  ...
  float L0 =
      cusp.x + 0.5 * (sign(Ld) * (e1 - sqrt(e1 * e1 - 2.0 * k * abs(Ld))));
...
vec2 get_ST_mid(float a_, float b_) {
  float S =
      0.11516993 + 1.0 / (+7.44778970 + 4.15901240 * b_ +
  ...
  float T =
      0.11239642 + 1.0 / (+1.61320320 - 0.68124379 * b_ +
...
vec3 get_Cs(float L, float a_, float b_) {
  ...
  float C_mid;
  {
    ...
    C_mid = 0.9 * k *
            sqrt(sqrt(1.0 / (1.0 / (C_a * C_a * C_a * C_a) +
                             1.0 / (C_b * C_b * C_b * C_b))));
  }

  float C_0;
  {
    // for C_0, the shape is independent of hue, so vec2 are constant. Values
    // picked to roughly be the average values of vec2.
    float C_a = L * 0.4;
    float C_b = (1.0 - L) * 0.8;
...
vec3 okhsl_to_srgb(vec3 hsl) {
  ...
  return vec3(srgb_transfer_function(rgb.r), srgb_transfer_function(rgb.g),
              srgb_transfer_function(rgb.b));
}

vec3 srgb_to_okhsl(vec3 rgb) {
  vec3 lab = linear_srgb_to_oklab(vec3(srgb_transfer_function_inv(rgb.r),
...
vec3 okhsv_to_srgb(vec3 hsv) {
  ...
  float scale_L =
      cbrt(1.0 / max(max(rgb_scale.r, rgb_scale.g), max(rgb_scale.b, 0.0)));
...
vec3 srgb_to_okhsv(vec3 rgb) {
  ...
  // we can then use these to invert the step that compensates for the toe and
  // the curved top part of the triangle:
  vec3 rgb_scale = oklab_to_linear_srgb(vec3(L_vt, a_ * C_vt, b_ * C_vt));
  float scale_L =
      cbrt(1.0 / max(max(rgb_scale.r, rgb_scale.g), max(rgb_scale.b, 0.0)));
